    ...
    const status = _([
        ...
        pkg.unused
            ? [
                  ...
                  indent +
                      `Depcheck did not find code similar to ${chalk.green(`require('${packageName}')`)} or ${chalk.green(`import from '${packageName}'`)}.`,
                  indent +
...
function outputConsole(currentState) {
    ...
    if (rows.length) {
        ...
        console.log(
            `Use ${chalk.green(`npm-check -${currentState.get("global") ? "g" : ""}u`)} for interactive update.`,
        );
        process.exitCode = 1;
    } else {
        console.log(
