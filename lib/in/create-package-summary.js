...
function createPackageSummary(moduleName, currentState) {
    ...
    const modulePackageJson = readPackageJson(join(modulePath, "package.json"));
    ...
    // Ignore sibling packages inside the same workspace. They are satisfied by
    // a symlink to a local directory, never published, and looking them up
    // would report a bogus "package could not be found" registry error.
    const workspacePackageNames = currentState.get("workspacePackageNames");
    if (workspacePackageNames?.includes(moduleName)) {
        return false;
    }
    ...
    if (ignore) {
        const ignoreMatch = Array.isArray(ignore)
            ? ignore.some((ignoredModule) =>
                  minimatch(moduleName, ignoredModule),
              )
    ...
    function foundIn(files) {
        ...
        return (
            ...
            files
                .map((filepath) =>
                    filepath.replace(currentState.get("cwd"), ""),
                )
                .join(", ")
        );
    }
